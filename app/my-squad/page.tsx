...
import { Download, RotateCcw, X, X as XIcon } from "lucide-react";
...
type ViewMode = "roster" | "formation";
...
export default function MySquadPage() {
  ...
  // メンバー選考とフォーメーション作成を、遷移せず1画面内のタブ切り替えで行き来できるようにする
  ...
  return (
    <div className="my-squad-page">
      ...
      <Tabs
        ...
        className="my-squad-page__view-tabs"
      >
        <TabsList>
          <TabsTrigger value="roster">メンバー選考</TabsTrigger>
          <TabsTrigger value="formation">フォーメーション</TabsTrigger>
        </TabsList>
      </Tabs>
          ...
          {members.length === 0 ? (
            ...
            <>
              ...
              <div className="my-squad-page__cta">
                <Button onClick={() => setView("formation")}>フォーメーションで配置する</Button>
              </div>
            </>
          )}
        </>
        ...
        <>
          {members.length === 0 ? (
            <p className="my-squad-page__empty-hint">
              ...
              <button
                type="button"
                className="my-squad-page__empty-link"
                onClick={() => setView("roster")}
              >
                「メンバー選考」タブ
              </button>
              でメンバーを追加してから、フォーメーションを組みましょう。
            </p>
          ) : (
            <>
