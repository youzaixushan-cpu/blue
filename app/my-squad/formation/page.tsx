...
export default function FormationMakerPage() {
  const {
    members,
    removeMember,
    formationId,
  ...
  return (
    <div className="my-squad-page">
      <SectionHeading
        eyebrow="Formation Maker"
        title="フォーメーションメーカー"
        description="あなたの26人をフォーメーションの枠にドラッグして配置しましょう。"
        action={
      ...
      {members.length === 0 ? (
        <p className="my-squad-page__empty-hint">
          まだ誰も追加されていません。
          <Link href="/my-squad" className="my-squad-page__empty-link">
            あなたの26人
          </Link>
          のページでメンバーを追加してから、フォーメーションを組みましょう。
        </p>
      ) : (
        <>
